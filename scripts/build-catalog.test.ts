...
import { ROUTE_CATALOG_SCHEMA_VERSION } from "../src/lib/presets";
...
describe("buildCatalog", () => {
  it("stamps version + generatedAt onto a validated catalog", () => {
    ...
    expect(catalog.schemaVersion).toBe(ROUTE_CATALOG_SCHEMA_VERSION);
  ...
  it("names the artifact by schema version so a v2 can coexist with v1", () => {
    expect(CATALOG_FILENAME).toBe(`presets.v${ROUTE_CATALOG_SCHEMA_VERSION}.json`);
  });
});
